...
\title{Multi-Omics of Maize Chromosomal Inversion $\textit{Inv4m}$ in Phosphorus Deficiency Show Typical Starvation Responses and Leaf-Age Dependency, Rather Than Adaptive Contributions From the Inverted Karyotype.}
...
Teosinte \mex introgression probably facilitated adaptation to temperate zones and further expansion worldwide \cite{yang2023, guo2018-on, barnes2022}.
...
Highland-associated chromosomal inversions, such as \invfour and \textit{Inv9f}, are prevalent in highland teosinte populations \cite{pyhajarvi2013} and traditional Mexican maize varieties (TVs) \cite{crow2020,gonzalez-segovia2019-jy} but are rare in modern temperate maize.
...
The inversion shows suppressed recombination, as confirmed in a biparental cross \cite{gonzalez-segovia2019-jy}.
\invfour demonstrates classic patterns of gene-by-environment interactions indicative of local adaptation.
Plants carrying the \invfour-highland allele show delayed flowering at low elevations and earlier flowering at high elevations \cite{gates2019-xu, barnes2022}.
...
\textit{Arabidopsis} \cite{luquez2006} and rice \cite{Mondal1984} show sequential or acropetalous senescence \cite{Leopold1961}, where aging progresses linearly from the base to the apex.
In maize, however, the vertical profile of physiological activity follows a bell-shaped curve whose peak depends on the developmental stage \cite{Ciganda2008}.
After flowering,  chlorophyll content \cite{Ciganda2008}, leaf area and longevity \cite{ValentinuzTollenaar2004}, nitrogen \cite{Wei2025} and water content \cite{GAo2023} first increase and then decline with leaf position, peaking near the ear leaf.
In contrast, during the vegetative phase, peak chlorophyll content and metabolic activity reside at the uppermost fully expanded 'collar' leaf \cite{Ciganda2008}.
While the steepness of this gradient is genotype-dependent \cite{Gao2023}, this architecture generally drives an 'outside-in' senescence pattern where aging proceeds from both the top and bottom of the canopy toward the middle \cite{Wei2025}.
This physiological architecture ensures that leaves closest to the ear, the primary sink, retain longevity and photosynthetic activity the longest \cite{ValentinuzTollenaar2004}.
This delay in senescence near the ear aligns with the principle of proximity allocation, minimizing transport distances for assimilates and optimizing energy use during the critical grain-filling period \cite{Wei2025, ValentinuzTollenaar2004}.
In the vegetative stage, the collar leaf is a source of photosynthates rather than a sink \cite{Turgeon1989}, which makes the senescence gradient centered at the collar leaf rather puzzling.
If we want to know the effect of leaf position on PSR, this is the developmental framework from which we can start to address the question.

Disentangling the phosphorus starvation response from this autonomous serial senescence gradient is complicated by the close relationship between nutrient status and leaf aging.
The physiological response to P-limitation can present a paradox: in two of three years of the field study by Plénet et al. (2000), P-deficiency delayed senescence instead of accelerating it \cite{Plenet2000}.
This maintenance of green leaf area is consistent with lipid remodeling mechanisms described in \textit{Arabidopsis}, where plants aggressively scavenge extraplastidic phospholipids (PC) for remobilization, yet strictly maintain thylakoid phosphatidylglycerol (PG) levels to preserve Photosystem II function \cite{Hagio2000, Kobayashi2009}.
Consequently, metabolic uncoupling occurs where the absence of visual chlorosis fails to capture the cryptic onset of lipid remobilization, a process further complicated by the natural senescence gradient of the canopy.
...
We found the classical response to phosphorus deficiency, where it caused a reduction in plant height, a delay in flowering, and a decrease in total kernel weight across all genotypes.
...
The gradient appears to accurately reflect the onset and initial phases of sequential senescence in our plants, which we estimated were in the V10–V12 stage, approximately 10 to 16 days prior to flowering. 
...
All these genes surpassed our statistical and effect size cutoffs for high-confidence DEGs \ref{table::}.


This developmental framework provides context for interpreting phosphorus starvation responses.
The leaf-stage variable represents a combined axis of photosynthetic capacity and the progression of developmental senescence, allowing us to quantify how nutrient stress interacts with natural developmental transitions during vegetative growth.
...
\subsection*{Some phosphorus starvation transcript responses increase in magnitude with leaf position}
...
These patterns validate the canonical phosphorus starvation response documented in previous studies \cite{Wang2020, He2021}.
...
In Arabidopsis, the ortholog XIPOTL1 affects flowering time, root architecture, and stress responses \cite{CruzRamirez2004}.
In maize, natural variation at the \textit{peamt2} locus associates with flowering time in Mexican highland populations \cite{PerezLimon2022, barnes2022}, suggesting that modulation of phospholipid biosynthesis contributes to adaptive variation in developmental timing.
...
These flowering-time regulators showed strong negative correlations with leaf stage, consistent with the natural progression from vegetative to reproductive phase transitions during our sampling window at V10-V12 stage.

\subsection*{LPE is a senecence suppressor in tomato and \textit{Arabidopsis}, but it might be a senescence marker in maize.}

 It has been demonstrated in 16:3 plants such as \textit{Arabidopsis} and tomato, that LPE acts as a leaf senescence suppressor \cite{amari}. 
 In these plants, exogenous LPE application delays senescence by inhibiting phospholipase D and preserving membrane integrity \cite{Farag1991, Kang2003}.
...
This divergence might reflect the difference between 16:3 and 18:3 plants in lipid metabolism architecture.
Maize, as an 18:3 plant, relies exclusively on the eukaryotic pathway for galactolipid synthesis \cite{Wang2020}, whereas 16:3 plants utilize both prokaryotic (plastid-localized) and eukaryotic (ER-localized) pathways.
In maize's eukaryotic pathway, diacylglycerol derived from phospholipid degradation is transported to the chloroplast outer envelope for galactolipid synthesis, creating direct metabolic flux from PE to LPE to DAG to MGDG.
...
Highly unsaturated TAG species were enriched, including TG54:9 and TG52:6, both containing 18:3 fatty acids.
This pattern correlates with the depletion of polyunsaturated phospholipids PC36:6 and LPC18:3, both of which showed leaf-stage effects.

The preservation of polyunsaturated fatty acids in TAG form is consistent with direct transfer via the PDAT pathway, which transfers intact acyl chains from phospholipids to diacylglycerol.
...
The fact that this metabolic strategy operates without significant transcriptional upregulation of PDAT genes indicates that the phospholipid-to-TAG flux is governed by substrate availability (increasing phospholipid degradation) rather than by increased PDAT gene expression.
...
This enzyme commits lipid flux toward TAG synthesis versus membrane lipid synthesis, and its age-dependent response is consistent with developmental control of lipid partitioning decisions.
Wang et al.
(2020) noted that TAG accumulation under phosphorus starvation likely represents transient storage for fatty acids released from membrane lipid catabolism, awaiting transport to younger tissues or conversion to energy via beta-oxidation.
...
Five Arabidopsis ortholog genes (SAGs) in~\cite{Zhang2014} are high-confidence DEGs for the effect of leaf stage, providing support for our observed leaf developmental gradient. 
...
To map the regulatory architecture governing these processes, we compiled a list of high-confidence SAGs by cross-referencing our DEGs with manually curated sources, resulting in a total of 110 genes. 
...
This divergence is particularly noteworthy because \textit{nactf132} has been linked to chlorophyll content regulation through \textit{nye1} activation, and natural variation in its 5$'$UTR associates with chlorophyll B levels~\cite{Wallace2014, Yuan2023}. 
...
\subsection*{Inv4m does not alter phosphorus starvation responses despite effects on flowering time}
...
The three genes showing significant genotype-by-phosphorus interactions (\textit{aldh2}, \textit{gras80}, and \textit{flz22}) are located outside the inversion boundaries and did not show additional interactions with leaf stage.
...
This independence indicates that \invfour's contribution to highland adaptation does not involve fine-tuning the coordination between leaf senescence and nutrient stress responses.
...
% responds to Leaf Zm00001eb191650 phos2        phosphate transporter2 log2FC =0.693   std err = 0.207   FDR =0.00963
The fact that genes like \textit{phos2}, a phosphate transporter located within the inversion and previously proposed as an adaptive candidate, do not show genotype-by-phosphorus interactions suggests that \invfour's adaptive value in highland environments comes primarily from coordinating developmental timing with the constrained growing season rather than from direct enhancement of nutrient stress tolerance.
...
Regarding a possible effect of inv4m in phosphorus response throuigh control of leaf senescence, the genotype shows negligible effects on senescence gene expression in this experiment. 
Only \textit{lkrsdh1} exhibits an \textit{Inv4m} main effect, and this gene lies outside the inversion boundaries without additional environment-dependent interactions. 
This pattern reinforces the idea that the adaptive value of \textit{Inv4m} likely operates through constitutive developmental timing rather than the modulation of environment-specific stress responses.
...
The divergence of lysophosphatidylethanolamine patterns from 16:0 plant models highlights the importance of lipid metabolism architecture differences between 16:0 and 18:3 plant species.
