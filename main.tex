...
\invfour is a 13 Mb inversion in maize that is particularly prevalent in highland maize and highland wild relatives from Mexico. Maize from the highlands of the Trans-Mexican volcanic belt has been shown to be well-adapted to the volcanic, acidic soils with low phosphorus availability. \invfour carries several genes involved in P acquisition and utilization. We therefore hypothesized that \invfour may be involved in adaptation to low phosphorus levels. To test this hypothesis, we introgressed a highland maize variety from the highlands of Michoacán, México, into the temperate line B73 and developed Near-Introgression Lines carrying \invfour. We then grew NILS carrying the inversion and controls in soils with low and high phosphorus and evaluated fitness effects of the inversion, as well as changes in gene expression using RNA-Seq.  
Here, we demonstrate that P starvation elicits highly conserved transcriptomic, lipidomic, and ionomic responses across near-isogenic lines that differ at the \invfour inversion. Thousands of genes, multiple lipid classes, and major nutrient pools respond strongly to P availability. 
...
% Adaptation of maize (Zea mays L.) to the Mexican highlands is driven by introgression from wild teosinte (Zea mays ssp. mexicana), most notably through the chromosomal inversion Inv4m.
% While Inv4m is strongly associated with clinal variation in flowering time, its potential role in adaptation to the phosphorus-fixing Andosols of the highlands remains unresolved. We evaluated the physiological and molecular impact of Inv4m using Near Isogenic Lines (NILs) grown under phosphorus-limiting field conditions.
% Phenotypic data showed that phosphorus deficiency reduced plant height and total kernel weight across all genotypes, while Inv4m exerted a constitutive effect on flowering time and height independent of nutrient status.
...
% This gradient revealed a metabolic uncoupling where the remodeling of membranesâcharacterized by the substitution of phospholipids with galactolipidsâproceeded prior to visual chlorosis.
% Contrary to the hypothesis that Inv4m confers specific nutrient stress tolerance, the inversion did not alter these conserved metabolic starvation responses.
% These findings suggest that Inv4m facilitates adaptation to highland environments through the regulation of developmental timing rather than by rewiring the fundamental metabolic response to low phosphorus.
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In contrast, the collar leaf is a source of photosynthates rather than a sink \cite{evert1996}, as it is completely mature, but this pattern might be indicative of its proximity to the main shoot carbon sinks of the vegetative phase: the fastest elongating internodes and immature leaves \cite{zhao2022}.
...
A greenhouse assay found evidence for nitrogen remobilization from old to young leaves in plants grown in phosphorus limitation \cite{usuda1995}. In this study, the phosphorus treatment had a small or null effect on the nitrogen and chlorophyll content of young leaves, while the older leaves presented a notable decrease of both.
...
\caption{
\textbf{Phosphorus Starvation Response is Largely Additive, with $\invfour$ Modulating Cob Diameter.}
\textbf{(A)} Experimental design showing the four sampled leaves from $\invfour$ and control (CTRL) NILs at 63 DAP ($\sim$ V13).
...
We observed the classical response to phosphorus deficiency, characterized by a reduction in plant height, a delay in flowering, and a decrease in total kernel weight across all genotypes.
...
We found a significant linear time dependence for -P relative reduction in biomass ($p = 0.027$) but the time effect was not significant for the inversion ($p = 0.90$), Fig.~\ref{fig:biomass}~B).
...
Lipid profiling shows typical changes associated with both leaf aging and phosphorus starvation (Fig.~\ref{fig:volcano_multiomics}~C-D, Table~\ref{table::differential_lipids}).
...
The core of the response involved the classic mechanisms of P mobilization and reallocation, which are conserved across plant species (Fig~\ref{fig:volcano_multiomics}~B, Table~\ref{table::leafDEGs}).
...
The significant interaction terms we observed for chlorophyll metabolism, photosynthesis, and senescence gene sets indicate that phosphorus deficiency does not uniformly shift all leaves along this gradient but rather amplifies the divergence between young and old tissues.

\subsection*{Starvation transcript responses mostly increase in magnitude with leaf position}
...
Gene Ontology enrichment for this set highlighted photosynthesis and light-harvesting complex terms, while KEGG analysis specifically identified light-harvesting complex genes rather than Calvin cycle components.
...
Genes with positive interaction terms showed amplified phosphorus-starvation responses in older leaves, corresponding to accelerated senescence and enhanced nutrient remobilization.
...
Gene Ontology enrichment for this positive interaction set included amino acid transport, aging, and detoxification processes.
...
The expression profiles we observed in older leaves under phosphorus limitation suggest accelerated progression of developmental senescence rather than young leaves experiencing moderate stress, indicating that nutrient limitation accelerates the natural senescence program during the vegetative phase.

\subsection*{PC34:2 depletion increases with leaf age and correlates with flowering delay}
...
The coordinate patterns we observed (PC34:2 depletion, flowering delay under phosphorus stress, and \textit{peamt2} suppression) raise the possibility that phosphorus stress influences reproductive timing through multiple mechanisms.
...
If ZCN8 florigen activity or stability requires PC34:2 binding, as demonstrated for Arabidopsis FT interactions with membrane lipids in temperature-dependent contexts, then systematic depletion of this specific phospholipid species could potentially impair florigen signaling beyond resource limitation alone.

The downregulation of MADS-box transcription factors \textit{zmm4} and \textit{zmm15} with increasing leaf age further connects lipid remodeling to developmental transitions.
...
This accumulation indicates that LPE acts as a marker for initial senescence, the inverse of its reported senescence-suppressor role in 16:3 plants.
...
\subsection{Senescence-Associated Gene Expression Compounds with Phosphorus Limitation in Older Leaves}
...
The ribonuclease \textit{rns} (\textit{Zm00001eb144680}) exemplifies this pattern; it exhibits both a strong main effect of phosphorus starvation and significant age-dependent amplification, suggesting that RNA degradation may accelerate specifically where developmental senescence and nutrient stress converge. 
...
To map the regulatory architecture governing these processes, we compiled a list of SAGs by cross-referencing our high-confidence DEGs with manually curated sources \cite{zhang2014c, liu2011,ojeda2026,berardini2015,durinck2005}, resulting in a total of 110 genes (Supplemantary File 1). 
...
While typical senescence regulators like \textit{nactf108} and \textit{nactf44} (\textit{Zm00001eb015630}) consistently increase with leaf age, \textit{nactf132} (\textit{Zm00001eb324550}) (ZmNAC132) shows opposing responses: strong downregulation under phosphorus stress but upregulation with leaf age. 
...
The contrasting responses suggest that developmental senescence and nutrient-stress-accelerated senescence might engage partially distinct regulatory networks despite converging on common downstream targets.
...
\subsection*{Inv4m does not alter phosphorus starvation responses}
...
The three genes showing significant genotype-by-phosphorus interactions (\textit{aldh2}, \textit{gras80}, and \textit{flz22}, Fig~\ref{fig:sup_interaction}) are located outside the inversion boundaries and did not show additional interactions with leaf stage.
...
\begin{figure}[!hb]
\centering
\includegraphics[width=\textwidth]{figs/Sup_biomass.png} % Assuming this is the filename
\caption{
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=\linewidth]{figs/Sup_ion.png}
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=\linewidth]{figs/Sup_interaction.png}
...
(\textbf{B}) \textbf{Manhattan plot for DEGs with an Inv4m $\mathbf{\times} -\mathbf{P}$ interaction}. Only $\textbf{3}$ DEGs show a significant interaction effect with the $\textit{Inv4m}$ inversion.  is linked to $\textit{aldh2}$ and $\textit{flz22}$.
