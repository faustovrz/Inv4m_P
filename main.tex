...
While the photosynthesis genes \textit{pep1} and \textit{ssu1} (pep carboxylase and rubisco small subunit, respectively)  were downregulated, the senescence-associated genes \textit{salt1} and \textit{mir3}  were upregulated.
...
\subsection*{Phosphorus Starvation Responses Increase in Magnitude with Leaf Position}
...
LION enrichment analysis confirmed these systemic changes: triacylglycerols showed strong enrichment, while glycerophospholipids and membrane components were significantly depleted.
...
\subsection*{LPE might be a Senescence Marker in Maize, but is a Senecence Suppressor in Tomato and Arabidopsis}
...
\subsection*{Senescence Gene Expression Compounds with Phosphorus Limitation in Older Leaves}

Multiple independent evidence streams validate the developmental senescence component of the leaf-position gradient and its interaction with phosphorus stress.
The GO-senescence gene set showed both a strong Leaf main effect and a significant Leaf $\times$ -P interaction.
We identified 94 combined senescence DEGs by integrating LSD database genes, SAG orthologs from literature, and Arabidopsis homologs.
Among these, 33 high-confidence SAG ortholog DEGs showed significant responses across predictors.
The E3 ubiquitin ligase family was represented by 14 UPL genes involved in protein turnover during senescence.
NAC and WRKY transcription factors, which are master regulators of senescence programs, included 5 DEGs characterized by Li et al.
(2024) as maize senescence regulators.
Chlorophyll degradation enzymes increased with leaf developmental position, while chlorophyll synthesis and carotenoid biosynthesis declined, patterns validated by Ciganda et al.
(2008), who reported chlorophyll content decline in basal maize leaves.

The coordinated response across omics platforms shows the integration of developmental senescence programs with phosphorus limitation.
Senescence-associated gene expression increases with age, phospholipids deplete, triacylglycerols accumulate, and photosynthesis declines (and all of these responses amplify under phosphorus stress in older leaves during the vegetative phase).
This coordination supports the interpretation that phosphorus deficiency does not impose a separate stress response but rather accelerates the natural senescence program that normally leads to death of the lowest leaves.

Ionomic data provide whole-plant context for these molecular patterns.
The seed-to-stover phosphorus ratio increased under phosphorus deficiency, indicating prioritized allocation to reproductive sinks at the expense of vegetative tissue maintenance.
This resource allocation strategy reflects a form of developmental triage: older leaves positioned lower in the canopy are sacrificed through accelerated senescence to support younger tissues and reproductive structures.
The strong correlation between leaf developmental position and response magnitude suggests that plants employ this triage strategy systematically during phosphorus limitation, allowing senescence of older leaves to sustain functions in actively growing tissues.
