...
\caption{\textbf{Ionomic responses of \invfour
   and control (CTRL) maize lines under         
  phosphorus sufficiency (+P) and deficiency
  (-P).}                                        
  Boxplots for phosphorus (P), zinc (Zn),
  calcium (Ca), and sulfur (S) show, per        
  genotype, \textbf{(A)} element concentrations
  (mg/kg) in stover and grain, \textbf{(B)}     
  grain/stover concentration ratios,
  \textbf{(C)} per-plant content (mg/plant) in
  stover and grain, and \textbf{(D)} mineral
  harvest index (HI, grain/total).
  Ca concentrations are plotted on a log scale.
  Brackets show $-$P vs $+$P contrasts within
  each genotype, computed from \texttt{emmeans} 
  on the per-response linear model $y \sim
  \text{Genotype} \times \text{Treatment}$, with
   Benjamini--Hochberg FDR pooled across all
  brackets drawn on the figure: $\textit{FDR} <
  0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $<
  0.0001$ (****); non-significant brackets are
  not shown.
  Effect sizes and exact \textit{p-values} are
  reported in Table~\ref{table::ionome}         
  \emph{(update ref as needed)}.}
...
Brackets show FDR-adjusted p-values for the marginal $\pm$P emmeans contrast
(Benjamini-Hochberg, pool of 48 contrasts across 16 phenotypes; see Methods).
...
kernel size.
Significance thresholds: n.s.\ not significant, $p < 0.05$ (*), $p < 0.01$ (**),
$p < 0.001$ (***), $p < 0.0001$ (****).
}
...
\begin{figure}[!hb]
\centering
\includegraphics[width=\textwidth]{figs/Figure_S3.png} % Assuming this is the filename
\caption{
...
\label{supp:Figure_S3} % Label for your figure
\end{figure}
...
Brackets show $-$P vs $+$P contrasts within each genotype, computed from \texttt{emmeans} on the per response linear model $y \sim \text{Genotype} \times \text{Treatment}$, with Benjamini and Hochberg \textit{FDR} pooled across all brackets drawn on the figure: $\textit{FDR} < 0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $< 0.0001$ (****); non significant brackets are not shown.
...
For concentration and content, a single pool is shown per cell — the pool where the \invfour vs CTRL contrast is significant under the unified FDR, or \textit{stover} when neither pool is significant — and the selected pool is indicated on the facet strip.
Brackets correspond to the \invfour $-$ CTRL contrast from \texttt{emmeans} on the per-response linear model $y \sim \text{Genotype} \times \text{Treatment}$, pooled across phosphorus treatments, with Benjamini--Hochberg \textit{FDR} applied once across all 20 cells: $\textit{FDR} < 0.05$ (*), $< 0.01$ (**), $< 0.001$ (***), $< 0.0001$ (****); non-significant brackets are not drawn.}
